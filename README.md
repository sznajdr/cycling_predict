...
Combines up to six pre-race signals (specialty scores with finish-type blending, cross-race recent form, historical stage results, frailty estimates, tactical HMM state, GC relevance) into softmax probabilities over the full startlist. Flat stages blend `sp_sprint` (65%) with `sp_one_day_races` (35%) so that puncher/classics riders are not systematically undervalued relative to pure bunch sprinters. Field reduction floors non-contenders' raw scores on flat (top 35) and hilly (top 30) stages so probability mass concentrates on realistic winners. The specialty signal uses quality-weighted rank percentile from actual DB results (UCI category weights: Grand Tours 1.3×, WT 1.0×, ProSeries 0.5×, continental 0.2×), falling back to static PCS specialty until 200+ Grand Tour stage results are present in the DB.

**Platt-calibrated temperature:** softmax temperature is fitted by maximum-likelihood on 343 historical stages (T*=4.9 flat, 9.3 mountain, 12.7 ITT) rather than a heuristic target range, followed by a Platt sigmoid that corrects absolute probability scale. Top-favourite win probabilities are now 5–7% on flat stages (matching historical base rates), rather than the pre-calibration 25–30%. Refresh calibration after adding new historical data: `python scripts/calibrate_stage_model.py`.

Uphill finish detection adjusts specialty blending and applies power-to-weight for mountain stages. Joins live Betclic odds, computes edge in basis points, and sizes stakes via half-Kelly. Signals with no data are excluded; weights renormalize automatically.
